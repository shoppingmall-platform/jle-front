import { Box, Button, Grid, Typography , ThemeProvider, createTheme, Divider, List} from '@mui/material'
    ...
    <ThemeProvider theme={localTheme}>
        ...
        <div>
            <Box sx={{width:'100%', marginBottom:1.5}}>
                <Typography sx={{fontSize:'10px', color:'black', textWrap:'wrap'}}>
                    로그인을 하시면 <br /> 더 많은 혜택을 받으실 수 있어요.
                </Typography>
                <Grid container sx={{marginTop:1.5}} >
                    <Grid item xs={6}>
                        <Button size='small' sx={{ backgroundColor:'#A3A09F', color:'white',width:'90%', fontSize:'small'}}>LOGIN</Button>
                    </Grid>
                    <Grid item xs={6}>
                        <Button size='small' sx={{ width:'90%', fontSize:'small'}}>Join Us</Button>
                    </Grid>
                    {buttonItems.map((item, index) => (
                        <Grid item xs={3} key={index} sx={{marginTop:1.5}}>
                            <Button sx={{ width: '100%', fontSize: 'x-small', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                {item.icon}
                                {item.label}
                            </Button>
                        </Grid>
